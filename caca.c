...
// XXX: https://www.hackerearth.com/practice/data-structures/trees/heapspriority-queues/practice-problems/algorithm/monk-and-iq/
...
#define MAX_CURSOS ((int)1E5)
typedef struct curso {
	natural id;
	listilla_fifo *ultimos;
	natural alumnos_cnt;
	natural suma_iq;
	entero_largo_sin_signo prioridad;
} curso;

curso cursos[MAX_CURSOS + 1] = { 0 };

static inline void caca_anade_alumno(curso *c, natural iq) {
	if (c->alumnos_cnt >= 2) {
		natural desc = (entero_largo) list_remove_element(c->ultimos);
		c->suma_iq -= desc;
	}
	list_add_element(c->ultimos, (void *) (entero_largo) iq);
	c->suma_iq += iq;
	c->alumnos_cnt++;
	c->prioridad = c->suma_iq * c->alumnos_cnt;
}
...
heap_shit_nodo_llave *caca_obten_llave(void *valor,
		heap_shit_nodo_llave *llave_res) {
	curso *a = valor;
	llave_res->contenido = &a->id;
	llave_res->contenido_tam = sizeof(a->id);
	return llave_res;
}
void *obten_prioridad(void *valor) {
	return valor;
}
char *curso_a_cadena(void *valor, char *buffer) {
	curso *a = valor;
	sprintf(buffer, "%u:%u:%u:%llu", a->id, a->alumnos_cnt, a->suma_iq,
			a->prioridad);
	return buffer;
}

static inline void caca_main() {
	natural c = 0;
	natural p = 0;
	natural n = 0;
	heap_shit *h = NULL;

	h = heap_shit_init(verdadero, compara_prioridad, caca_obten_llave,
			obten_prioridad, curso_a_cadena);

	scanf("%u %u %u\n", &c, &p, &n);

	for (natural i = 1; i <= c; i++) {
		curso *ca = cursos + i;
		ca->id = i;
		ca->ultimos = list_new();
	}
	...
	for (natural i = 1; i <= n; i++) {
		...
		caca_anade_alumno(ca, a);
	}

	for (natural i = 1; i <= c; i++) {
		curso *ca = cursos + i;
		heap_shit_insertar(h, ca);
	}
	for (natural i = 0; i < p; i++) {
		natural a = 0;
		curso *ca = NULL;
		scanf("%u", &a);
		ca = heap_shit_consulta_torpe(h);

		caca_anade_alumno(ca, a);

		heap_shit_actualiza(h, ca);
		if (i) {
			printf(" ");
		}
		printf("%u", ca->id);
